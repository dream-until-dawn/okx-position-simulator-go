...
func main() {
	var (
		...
		mode   = flag.String("mode", "trade", "trade=真实下单后逐字段对拍；check=只核对账户现有仓位的风险指标，不交易")
		wait   = flag.Duration("settle", 1200*time.Millisecond, "下单后等待账户状态落定的时间")
	)
	flag.Parse()

	if err := run(*mode, *instID, *lever, *sz, *envPos, *wait); err != nil {
		fmt.Fprintf(os.Stderr, "\n对拍失败: %v\n", err)
		os.Exit(1)
	}
}

func run(mode, instID, lever, baseSz, envPath string, settle time.Duration) error {
	...
	if mode != "trade" {
		return fmt.Errorf("未知的模式 %q，应为 trade 或 check", mode)
	}
