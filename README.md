...
> **状态：v1.2.0，API 已冻结。** 「近乎 100% 模拟」的确切含义、成立条件与已知边界，
> 见 [适用边界与保真度](docs/fidelity.md)；**出问题时不报错的那些**单列在
> [静默风险清单](docs/silent-risks.md)。
> 已可用：**正向与币本位** × **逐仓与全仓**四个象限的完整核算（开平加减仓与反手、
> 盈亏、手续费、全套风险指标）、内置撮合、**算法委托**、预下单计算与挂单冻结、
> 资金费结算、强平（含**逐仓与全仓两条阶梯减仓链路**，均经真实爆仓实测）。
>
> 未实测的只剩三条，且都不是「没做」：费率组优先级在模拟盘上结构性测不了，
> 算法委托的部分成交本库明确不建模，买卖模式反手的**账单条数**取数时模拟盘的
> 账单管线落后数小时（反手本身已实测）。见 [版本排期](docs/roadmap.md)。

## 从 v1.1.x 升级到 v1.2.0

**修了一个会静默产出错仓位的缺陷**，行为因此改变。三处：

| 场景 | v1.1.x | v1.2.0 |
|---|---|---|
| 开平仓模式下超量平仓（10 张平 4 张多头） | 反手成 **6 张多头**（想平仓，仓位反而变大、方向没变） | **裁剪到 4 张后平光**，与 OKX 一致 |
| 平仓挂单触发时仓位已归零 | 成交，开出反向的幽灵仓位 | **撤销**，原因 `CancelPositionGone` |
| `Fill` 报来一笔空仓上的平仓成交 | 静默空转 | **报错** |

前两条都实测有据：裁剪见 [okx-rules.md §9](docs/okx-rules.md)（1.50 张平 0.15 张
的仓位，`sz` 被裁到 0.15）；撤销见同节（137.89 张空头上的四笔 `buy/short`，
第一笔平光后其余三笔被 OKX 撤销，`cancelSource=22`）。

**新增了一个 `CancelReason` 取值 `CancelPositionGone`。** 对下游这是破坏性的
——穷尽处理 `CancelReason` 的 `switch` 会静默漏掉新分支。它的
`AffectsAccountState()` 为 **真**：策略若以为那层平仓单还挂着，会永远等一个
不会来的成交。

**买卖模式的反手不受影响**，那是正确行为，v1.2.0 用真实成交把它锁住了
（`net-reversal.json`）。
...
**零配置的默认组合变了**，这是本版唯一的破坏性变更：

| | v1.0.x | v1.1.0 |
|---|---|---|
| `Config.PosMode` 留空 | 买卖模式 | **开平仓模式** |
| `Fill`/`Order` 的 `TdMode` 留空 | 报错 | **逐仓** |

前者与 OKX 一致（实测账户 `posMode = long_short_mode`），也是回测最常用的组合；
后者是纯增量——此前留空直接报错，没有任何既有行为因此改变。

**要保持旧行为，显式声明即可：**

```go
sim, err := okxsim.New(okxsim.Config{
    PosMode: types.NetMode,   // v1.0.x 的默认值
    RefData: snap,
})
```

**不改会怎样**：开平仓模式要求每笔成交显式给 `long` 或 `short`，留空**当场报错**
（51000）。所以升级后要么立刻看到错误、要么本来就没依赖这个默认值——不会出现
「跑通了但口径变了」的情形。
...
`MetricsAt` 给出**假设价格**下的风险指标且不改动任何状态——压力测试要问
「价格到 X 时爆不爆」，不必再改掉标记价算完又改回来。

---

## 状态存档

参数扫描的断点续跑、事件重放、把中途状态存下来事后复盘：
...
**完整性是它存在的理由。** 手写搬运代码最容易漏掉挂单与算法委托——漏了不会报错，
只会让续跑时凭空少几笔在途委托，而回测照样跑得出结果。`State` 把八项可变状态
一次装齐：现金、仓位、三条行情、杠杆设置、挂单、算法委托（含移动止损已经棘轮到
哪里）。

持仓方式或规则数据版本不匹配时 `Restore` 直接报错——档位表若变了，同一个仓位的
维持保证金率就不一样，将就着跑只会得到一份看不出问题的错结果。
...
逐仓的结算方式取自**两次真实强平**的逐字段核对：一次全部强平，一次完整的
`阶梯减仓 → 全平 → 超额退回`。现金余额 `balChg` 全程为 0，损失恰好等于仓位保证金，
扣穿的部分由风险准备金退回。

全仓是**结算币种级**的：同币种的全仓仓位共担一份权益，一旦触发就一并了结——
因此推进某个合约的 K 线后，必须把该结算币种整体再看一眼。

**全仓这条路径已于 2026-09-01 由一次真实爆仓实测确证**，含阶梯减仓。它与逐仓
**不是同一套**：
...
| 全部强平 | `105` | `104` |
| 损失落点 | 仓位保证金（`balChg` 恒为 0）| 现金余额 |
| 账单的 `pnl` | 纯盈亏 | **含罚金** |

最后一行是解析上的陷阱：拿同一套解析读两种账单会把罚金算重或算漏。本库的
`Liquidation` 把 `Pnl` 与 `Penalty` 分开报，两种模式一致。
...
**验收标准要真的被验证，不是写下来就算数。** design.md 的第一条标准是「字段级同构」，
而它在 v0.8.0 之前从未被真正验证过——对拍工具比的是手挑的十几个字段。补上全字段
对拍的当天就暴露出 35 处差异与 4 个从未建模的字段。写下标准与验证标准之间的距离，
比想象中远。
...
```
cd cmd/conformance
go run . -mode view                      # 与 OKX 原始 JSON 逐字段对拍，不交易
go run . -mode check                     # 核对现有仓位的风险指标，不交易
go run . -inst BTC-USDT-SWAP -lever 5    # 真实下单后逐字段对拍
go run . -mode adjust -sz 0.2            # 在现有仓位上加减仓，含累计量
go run . -mode margin -inst ETH-USDT-SWAP -sz 2   # 逐仓保证金的增减
```

**`view` 是那条验收标准本身。** 另外四种模式比的是手挑的字段，它比的是整个响应：
OKX 返回的每一个字段，要么本库建模了并且值对得上，要么在 `policy.go` 里写明为什么
不建模。两样都没有就判失败——挑着比永远发现不了「有个字段我压根没建模」。

实盘 **184 个字段：有差异 0，未分类 0**（其中 20 个仅差在精度——OKX 用 float64 算并
截断，本库用 decimal 精确算，相对差 `1e-14` 量级；单独计数是为了让它的规模一直
看得见）。首跑曾暴露 35 处差异，包括「零与空串搞混」「`pnl` 一直空着」这类真错。

`check` 能覆盖下单流程难以构造的情形，且四个象限都走得通——实测一次同时核对币本位
的全仓空头与逐仓多头、外加两个距强平 1.2% 的 40 倍杠杆正向逐仓多空仓位，
27 个字段全部一致。
...
- [**适用边界与保真度**](docs/fidelity.md) —— 「近乎 100% 模拟」在什么条件下成立、
  验证到什么程度、哪些是取舍、哪些还没有证据。**先读这一份**
