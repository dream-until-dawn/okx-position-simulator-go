...
> **状态：v0.5.0。**
> 已可用：逐仓与**全仓**永续的完整核算（开平加减仓与反手、盈亏、手续费、
> 全套风险指标）、内置撮合、预下单计算与挂单冻结、资金费结算、强平。
> 币本位与算法单在后续版本，详见 [版本排期](docs/roadmap.md)。
...
三种模式互补。`check` 能覆盖下单流程难以构造的情形，且逐仓与全仓都走得通——
实测一次同时核对全仓仓位与两个距强平 0.6% 的 40 倍杠杆逐仓多空仓位，
**20 个字段全部一致**。
