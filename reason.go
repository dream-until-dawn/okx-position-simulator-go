...
//
// 这条分类由本库提供而不是让调用方自己列，有两层理由，第二层是下游补的：
//
//	一  本库知道哪些原因是账户级的，调用方只能猜
//	二  调用方的白名单是【保守猜测】，代价是**假阳性**——本库哪天加一种新的、
//	    其实无害的原因，在调用方那边会落进「未知」而触发停机。那是个**错的停机**，
//	    而且同样不报错：报告里只有一句「已停机」，看起来像真出事了
//
// 有了这个方法，适配层就能把新原因**如实翻译**而不是一律兜到未知，
// 假阳性与假阴性一起消掉——那是调用方单靠自己的白名单做不到的。
//
