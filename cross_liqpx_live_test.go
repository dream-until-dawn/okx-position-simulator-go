...
//
// 这份数据的分量在于它是**闭环**的：代入 API 报的 cashBal（只有 6 位小数）
// 相对差 9.6e-12，而代入由三笔资金费推算出的精确现金
// 286.8952266692395027（见 funding-short-negative.json）相对差降到 1.18e-16
// ——即 OKX 自己打印精度的最后一位。于是资金费公式与强平价公式互为对方的
// 验证：任何一边错了，这个数都对不上。
//
// # 只有结算币在兜底
//
