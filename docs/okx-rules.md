...
在模拟盘上以 100 倍杠杆开小额仓位并等到真实触发，取得完整数据。
样本：BTC-USDT-SWAP 空头 0.15 张 @ 78099，保证金 1.171485，强平价 78526.61。
...
提交 1.50 张去平一个 0.15 张的仓位，OKX **不拒绝**，而是把 `sz` 直接裁剪到
0.15 后成交（订单记录里的 `sz` 就是 0.15）。

#### 尚待实测确认

**阶梯减仓路径。** 触发它需要跨越多个档位的仓位。已在 FIL-USDT-SWAP 上布好
二档纵深的双向仓位（500000 张，一档上限的两倍，降档将砍掉 250000 张）等待触发。
该路径依据 OKX 的阶梯档位规则实现，尚未验证。
